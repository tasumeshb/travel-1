    ...
    export default {
        ...
        created: function () {
            ...
            for(var i = 0; i < this.schema.settings.length ; i++){
                ...
                var fieldId = this.schema.settings[i].id;
                this.fakeModel[fieldId] = (this.schema.settings[i].type === 'input' || this.schema.settings[i].type === 'textArea') ? '' : null;
            }
            this.normalizeListItems();
        },
        watch: {
        ...
        methods:{
            normalizeListItems(){
                ...
                this.value.forEach(function(item){
                    if (typeof item._active === 'undefined') {
                        me.$set(item, '_active', false);
                    }
                    me.schema.settings.forEach(function(field){
                        if(typeof item[field.id] === 'undefined'){
                            me.$set(item, field.id, (field.type === 'input' || field.type === 'textArea') ? '' : null);
                        }
                    });
                });
            },
