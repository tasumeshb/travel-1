(function ($) {
    function getPhone(form) {
        var input = form.find('input[name=phone]')[0];
        if (window.BravoPhoneIntl && input) {
            return window.BravoPhoneIntl.getNumber(input);
        }
        return form.find('input[name=phone]').val() || '';
    }

    $('.bravo-form-register-vendor [type=submit]').click(function (e) {
        e.preventDefault();
        var form = $(this).closest('.bravo-form-register-vendor');
        $.ajaxSetup({
            headers: {
