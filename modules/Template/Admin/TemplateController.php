...
class TemplateController extends AdminController
{
    ...
    public function store(Request $request)
    {
        ...
        $lang = $request->input('lang', get_main_lang());
        $preparedContent = $this->prepareTemplateContentForSave($request->input('content'));
        $siteLocale = setting_item('site_locale') ?: get_main_lang();

        $template->title = $request->input('title');
        $template->content = $preparedContent;
        ...
        if (is_enable_multi_lang()) {
            $translation = $this->templateTranslationClass::query()
                ->where('origin_id', $template->id)
                ->where('locale', $lang)
                ->first();

            if (!$translation) {
                $translation = new $this->templateTranslationClass();
                $translation->origin_id = $template->id;
                $translation->locale = $lang;
            }

            $translation->title = $template->title;
            $translation->content = $preparedContent;
            $translation->save();

            // Frontend reads translation for app locale; keep main row aligned with site default.
            if ($lang === $siteLocale) {
                $this->templateClass::query()
        ...
        if (method_exists($template, 'saveSEO')) {
            $template->saveSEO($request, is_default_lang($lang) ? null : $lang);
        }
    ...
    /**
     * Normalize block models before persisting (e.g. sync slider bulk URLs into list_slider).
     */
    protected function prepareTemplateContentForSave($content): string
    {
        ...
        foreach ($items as &$item) {
            if (!is_array($item['model'] ?? null)) {
                continue;
            }

            if (($item['type'] ?? '') === 'form_search_all_service') {
                $item['model'] = FormSearchAllService::syncSliderLinksInModel($item['model']);
            }
