...
<div class="panel">
    ...
    <div class="panel-body">
        @if(is_default_lang())
            <div class="row">
                @include('Core::admin.components.price-currency-select', ['row' => $row])
                @php
                    $hourPrice = service_price_for_admin($row, 'price_per_hour');
                    $dayPrice = service_price_for_admin($row, 'price_per_day');
                @endphp
                <div class="col-lg-6">
                    <div class="form-group">
                        <label class="control-label">{{__("Price per hour")}}</label>
                        <input type="number" step="any" min="0" name="price_per_hour" class="form-control" value="{{ $hourPrice['amount'] !== '' ? $hourPrice['amount'] : '' }}" placeholder="{{__("Price per hour")}}">
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="form-group">
                        <label class="control-label">{{__("Price per day")}}</label>
                        <input type="number" step="any" name="price_per_day" class="form-control" value="{{ $dayPrice['amount'] !== '' ? $dayPrice['amount'] : '' }}" placeholder="{{__("Price per day")}}">
                    </div>
                </div>
