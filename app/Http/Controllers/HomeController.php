    ...
    public function getLocation(Request $request)
{
    ...
    // Make a server-side request to Google Geocoding API
    $client = new Client();
    $apiKey = 'AIzaSyD_ivQdtQK5UuTcspqtw5u5_r27Xbi_f9w'; // Store API key in .env file
    ...
    try {
        ...
        if ($data['status'] === 'OK') {
            ...
            return response()->json(['status' => 'error', 'message' => 'Geocoding failed', 'data' => $data], 400);
        }
    } catch (\Exception $e) {
        // Handle exceptions
        return response()->json(['status' => 'error', 'message' => 'Request failed', 'error' => $e->getMessage()], 500);
    }
}
