...
class Currency
{
    ...
    public static function getExchangeRateForCurrency($currencyCode)
    {
        $currencyCode = strtolower((string) $currencyCode);
        $main = strtolower((string) setting_item('currency_main'));

        if ($currencyCode === '' || $currencyCode === $main) {
            return 1.0;
        }

        foreach (static::getActiveCurrency() as $item) {
            if (strtolower((string) ($item['currency_main'] ?? '')) === $currencyCode) {
    ...
    public static function convertFromCurrencyToMain($price, $fromCurrency)
    {
        if (!is_numeric($price)) {
            return 0;
        }

        return (float) $price * static::getExchangeRateForCurrency($fromCurrency);
    }

    public static function convertFromMainToCurrency($price, $toCurrency)
    {
        if (!is_numeric($price)) {
            return 0;
        }

        $rate = static::getExchangeRateForCurrency($toCurrency);
        if ($rate <= 0) {
            return (float) $price;
        }
    ...
    public static function getAdminCurrencyOptions()
    {
        $options = [];
        foreach (static::getActiveCurrency() as $item) {
            $code = strtolower((string) ($item['currency_main'] ?? ''));
            if ($code === '') {
                continue;
            }
            $currency = static::getCurrency($code);
            $options[$code] = $currency['name'] ?? strtoupper($code);
        }
